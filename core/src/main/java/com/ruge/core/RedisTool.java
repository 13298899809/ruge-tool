...
//            jedisPoolConfig.setTestOnBorrow(SysConfigUtil.getSysConfigUtil("redis.properties").getBoolean("redis.testOnBorrow"));
     ...
     * @param key key
     * @param value value
     * @return 设置key value,如果key已经存在则返回0  存在不做任何操作 不存在就添加
     */
     ...
     * @param offset offset
     * @param str 字符串
     * @return 通过key 和offset 从指定的位置开始将原先value替换
     */
    ...
    /**
     * @param keysValues keysValues
     * @return 批量的设置key:value,可以一个,如果key已经存在则会失败,操作会回滚
     */
     ...
     * @return 通过下标 和key 获取指定下标位置的 value
     */
     ...
     *
     * @param key key
     * @return 通过key 对value进行加值+1操作,当value不是int类型时会返回错误,当key不存在是则value为1
     */
     ...
     * @return 通过key给指定的value加值,如果key不存在,则这是value为该值
     */
     ...
     *
     * @param key key
     * @return 对key的值做减减操作,如果key不存在,则设置key为-1
     */
     ...
     * @param field
     * @param value
     * @return 通过key给field设置指定的值,如果key不存在则先创建,如果field已经存在,返回0
     */
     ...
     * @param field
     * @param value
     * @return 通过key给field设置指定的值,如果key不存在,则先创建
     */
